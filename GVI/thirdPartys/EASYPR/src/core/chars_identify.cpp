...
namespace easypr {
    ...
    CharsIdentify::CharsIdentify() {
        LOAD_ANN_MODEL(ann_, qApp->applicationDirPath().toStdString() + kDefaultAnnPath);
        LOAD_ANN_MODEL(annChinese_, qApp->applicationDirPath().toStdString() + kChineseAnnPath);
        LOAD_ANN_MODEL(annGray_, qApp->applicationDirPath().toStdString() + kGrayAnnPath);

        kv_ = std::shared_ptr<Kv>(new Kv);
        kv_->load(qApp->applicationDirPath().toStdString() + kChineseMappingPath);

        extractFeature = getGrayPlusProject;
    }

    void CharsIdentify::LoadModel(std::string path) {
        if (path != std::string(qApp->applicationDirPath().toStdString() + kDefaultAnnPath)) {
            if (!ann_->empty())
                ann_->clear();
    ...
    void CharsIdentify::LoadChineseModel(std::string path) {
        if (path != std::string(qApp->applicationDirPath().toStdString() + kChineseAnnPath)) {
    ...
    void CharsIdentify::LoadGrayChANN(std::string path) {
        if (path != std::string(qApp->applicationDirPath().toStdString() + kGrayAnnPath)) {
            if (!annGray_->empty())
                annGray_->clear();
