...
namespace easypr {
    ...
    PlateJudge::PlateJudge() {
        bool useLBP = false;
        if (useLBP) {
            LOAD_SVM_MODEL(svm_, qApp->applicationDirPath().toStdString() + kLBPSvmPath);
            extractFeature = getLBPFeatures;
        }
        else {
            LOAD_SVM_MODEL(svm_, qApp->applicationDirPath().toStdString() + kHistSvmPath);
            extractFeature = getHistomPlusColoFeatures;
        }
    }

    void PlateJudge::LoadModel(std::string path) {
        if (path != std::string(qApp->applicationDirPath().toStdString() + kDefaultSvmPath)) {
            if (!svm_->empty())
                svm_->clear();
